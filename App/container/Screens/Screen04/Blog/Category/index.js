  ...
  _getPostList() {
    ...
    NetInfo.isConnected.fetch().then(isConnected => {
      if (isConnected) {
        ...
        fetch(
          Globals.SITE_URL +
            "get_category_posts/?id=" +
            categoryid +
            "&page=" +
            this.state.page,
          config
        ) //"get_category_posts/?id="+categoryid,config)
                ...
                try {
                  if (responseJson.posts[i].categories.length > 1) {
                    for (
                      ...
                      if (
                        responseJson.posts[i].categories[j].id ==
                        this.state.CatID
                      ) {
                        // searchData.push({
                        //     id: responseJson.posts[i].id,
                        ...
                        console.log(this.state.CatID);
                        console.log(
                          "id : " + responseJson.posts[i].categories[j].id
                        );
                      }
                    }
                  } else {
                    if (
                      responseJson.posts[i].categories[0].id == this.state.CatID
                    ) {
                      // searchData.push({
                      //   id: responseJson.posts[i].id,
                      //   title: responseJson.posts[i].title,
                      ...
                      console.log(this.state.CatID);
                      console.log(
                        "id : " + responseJson.posts[i].categories[0].id
                      );
                    }
                  }
                } catch (error) {}
          ...
          .then(responseJson => {
            ...
            if (responseJson.posts.length != 0) {
              ...
              for (var j = 0; j < tempData1.length; j++) {
                ...
                if (likedPosts.length != 0) {
                  for (var i = 0; i < likedPosts.length; i++) {
                    if (likedPosts[i].id == tempData1[j].id) {
                      console.log(likedPosts[i].id);
          ...
          >
            ...
            >
              {rowData.isLiked == true ? (
                <TouchableOpacity
                  onPress={() => this.onSearchLikeClick(rowData.id)}
                  style={[styles.heart, { paddingLeft: Metrics.WIDTH * 0.02 }]}
                >
                  <Image
                    style={styles.likeBtn}
                  ...
                  <View>
                    ...
                    {this.state.isSearching != true ? (
                      <View>
                        ...
                        />
                        {totalpages <= this.state.page ? null : (
                          <TouchableOpacity
                            style={styles.more}
                            onPress={() => this.reload()}
                          >
                            <Text name="arrow-down" style={styles.moreTxt}>
                              Load More
                            </Text>
                          </TouchableOpacity>
                        )}
